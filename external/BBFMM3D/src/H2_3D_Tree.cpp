...
void H2_3D_Tree::FMMSetup(nodeT **A, double *Tkz,  double *Kweights,
    ...
    if (use_chebyshev) {
        sprintf(Kmat,"./%sCK%d.bin",kernelType.c_str(),n);
        sprintf(Umat,"./%sCU%d.bin",kernelType.c_str(),n);
        sprintf(Vmat,"./%sCV%d.bin",kernelType.c_str(),n);
    } else {
        sprintf(Kmat,"./%sUK%d.bin",kernelType.c_str(),n);
        sprintf(Umat,"./%sUU%d.bin",kernelType.c_str(),n);
        sprintf(Vmat,"./%sUV%d.bin",kernelType.c_str(),n);
    }
