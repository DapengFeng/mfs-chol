...
/*! \file compute.hpp
 */
#ifndef __old_compute_hpp__
#define __old_compute_hpp__
...
#pragma omp parallel for reduction(min:xmin_x, xmin_y, xmin_z) reduction(max:xmax_x, xmax_y, xmax_z)    
  for (int i = 0; i < Ns; ++i) {
    xmin_x = min(xmin_x, source[i].x);
    xmin_y = min(xmin_y, source[i].y);
    xmin_z = min(xmin_z, source[i].z);

    xmax_x = max(xmax_x, source[i].x);
    xmax_y = max(xmax_y, source[i].y);
    xmax_z = max(xmax_z, source[i].z);
  }

#pragma omp parallel for reduction(min:xmin_x, xmin_y, xmin_z) reduction(max:xmax_x, xmax_y, xmax_z)    
  for (int i = 0; i < Nf; ++i) {
    xmin_x = min(xmin_x, field[i].x);
    xmin_y = min(xmin_y, field[i].y);
    xmin_z = min(xmin_z, field[i].z);

    xmax_x = max(xmax_x, field[i].x);
    xmax_y = max(xmax_y, field[i].y);
    xmax_z = max(xmax_z, field[i].z);
  }

  // shift field and source pts s.t. they center around orign
  vector3 xmin;
  xmin.x = xmin_x; xmin.y = xmin_y; xmin.z = xmin_z;
  vector3 xmax;
  xmax.x = xmax_x; xmax.y = xmax_y; xmax.z = xmax_z;
