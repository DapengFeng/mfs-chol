...
void H2_3D_Tree::PrecomputeM2L(double *Kweights, double boxLen, double alpha,
  ...
  if (use_chebyshev) {
    sprintf(Kmat,"./%sChebK%d.bin", kernelType.c_str(), interpolation_order);
    sprintf(Umat,"./%sChebU%d.bin", kernelType.c_str(), interpolation_order);
    sprintf(Vmat,"./%sChebV%d.bin", kernelType.c_str(), interpolation_order);
  } else {
    sprintf(Kmat,"./%sUnifK%d.bin", kernelType.c_str(), interpolation_order);
