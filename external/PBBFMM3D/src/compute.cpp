...
template <typename T>
H2_3D_Compute<T>::H2_3D_Compute(T &FMMTree, const std::vector<vector3>& target, const std::vector<vector3>& source, double *charge, int nCols, double *output) {
    ...
    */
    ...
    FMMCompute(&FMMTree.tree,this->target, this->source,charge,FMMTree.K,FMMTree.U,FMMTree.VT,FMMTree.Tkz,FMMTree.Ktable,FMMTree.Kweights,FMMTree.Cweights,
                   FMMTree.homogen,&FMMTree.cutoff,FMMTree.interpolation_order,FMMTree.dof, output, FMMTree.use_chebyshev);
...
template class H2_3D_Compute<H2_3D_Tree>;
