...
int main(int argc, char *argv[])
{
  ...
  merge_meshes(Vq, Fq, Vb, Fb, VV, FF);
  {
    ...
    RmMatF_t V_vol;
    RmMatI_t F_vol;
    if ( igl::readOFF(pt.get<string>("solid_mesh.value"), V_vol, F_vol) ) {
      V_vol.rowwise() -= C;
      V_vol /= max_span;
      igl::writeOFF(string(OUTDIR+"/solid_mesh.off"), V_vol, F_vol);
    }
  }  
  ...
  menu_config.callback_draw_custom_window = [&]() {
    ...
    ImGui::Checkbox("jacobi_precond", &JACOBI_PRECOND);
    ImGui::End();
  };
