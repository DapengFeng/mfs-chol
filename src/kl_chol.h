...
struct cov_assembler
{
  ...
  void set_source_pts(const index_t n_pts, const real_t *h_xyz, const real_t *h_nxyz=NULL)
  {
    ...
    if ( h_nxyz ) { // there is normal info
  ...
  void debug(const KERNEL_TYPE ker_type) const;
...
class gpu_simpl_klchol
{
 public:
  ...
  pcg_ret_t pcg(const scalar_t *h_rhs, scalar_t *h_x,
                const bool preconditioned,
                const index_t maxits=1000,
                const real_t TOL=1e-6,
                const FMM_parameters *fmm_param=nullptr,
                std::vector<real_t> *residual=nullptr);

  void thread_safe_pcg_precomp(const FMM_parameters *fmm_param);
  pcg_ret_t thread_safe_pcg(const scalar_t *h_rhs, scalar_t *h_x,
                            const bool preconditioned,
  ...
  void evalKx_fmm(std::vector<scalar_t> &Kx, const FMM_parameters &param);
  void evalKx_fmm(scalar_t *Kx, const index_t n, const FMM_parameters &param);
  ...
  void predict_fmm(const int impulse_dim,
  ...
  // debug
  virtual void debug(const scalar_t *aux, const int aux_n);
 ...
 protected:
  ...
  // // buffer for triangular solve
  // void *d_sv_work_ = NULL;
  // size_t sv_buff_sz_;

  // FMM
  std::vector<vector3> sources_, targets_;
  std::vector<bbfmm3::vector3> srcs_, tgts_;  
  std::vector<scalar_t> charges_, potential_;
};
...
class gpu_super_klchol : public gpu_simpl_klchol<pde>
{
 public:
  ...
  void debug(const scalar_t *aux, const int aux_n);
