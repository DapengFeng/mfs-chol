...
using KDTree = nanoflann::KDTreeEigenMatrixAdaptor<klchol::RmMatF_t, 3, nanoflann::metric_L2>;
...
void determineChildren_3d(heapNode* const nodes, heapNode** const handles, ijlookup* const lookup, unsigned int* const parents, const double* const coords, const unsigned int N, const unsigned int Id, const unsigned int iter, const std::vector<double> &dist2bd) {
  const double pivotDist = nodes[0].dist;
  /*Need to save the kmin nad kmax beforehand, since otherwise the node might become its own parent, later on */
  const int kmin = lookup->i[parents[Id]];
  const int kmax = lookup->i[parents[Id] + 1];
  ...
  for (unsigned int k = kmin; (int)k < kmax; ++k) {
    ...
    if (tempDist2 < pivotDist * pivotDist) {
      double jDist = handles[lookup->j[k]]->dist;
      if (tempDist2 < jDist*jDist) {
        update(handles[lookup->j[k]], std::min(sqrt(tempDist2), dist2bd[lookup->j[k]]));
        jDist = std::min(sqrt(tempDist2), dist2bd[lookup->j[k]]);
      }
      ijlookup_newson(lookup, lookup->j[k]);
      if (sqrt(tempDist2) + jDist < pivotDist) {
        parents[lookup->j[k]] = iter;
...
void create_ordering_3d(unsigned int* P, unsigned int* revP, double* distances, const unsigned int N, const double* coords, unsigned int first_node, const KDTree *kdt) {
  /*Function to construct the ordering.
  * Inputs:
  *  P:
  *    An N element array containing the hierarchical ordering
  ...
  *the i-th entry of parents will contain the number of its parent in the ordering */
  ...
  std::vector<double> dist_bd(N, 1e16);
  if ( kdt ) {
    ...
    for (size_t k = 0; k < N; ++k) {
      ...
      res.init(&closest, &dist_to_bd);
      kdt->index->findNeighbors(res, &coords[3*k], nanoflann::SearchParams());
      dist_bd[k] = sqrt(dist_to_bd);
    }
  }
  ...
  for (unsigned int k = 0; k < N; ++k) {
    ...
    update(handles[k], std::min(dist_3d(rootId, k, coords), dist_bd[k]));
    parents[k] = 0;
  }
  timer.stop();
  std::cout << "update_handle time=" << timer.duration() << " ms\n";
  ...
  for (unsigned int k = 1; k < N; ++k) {
    ...
    P[k] = pivotId;
    revP[pivotId] = k;
    determineChildren_3d(nodes, handles, &lookup, parents, coords, N, pivotId, k, dist_bd);
  }
  timer.stop();
  std::cout << "determine_child time=" << timer.duration() << " ms\n";
